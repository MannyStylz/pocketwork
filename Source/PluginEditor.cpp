    ...
    : AudioProcessorEditor(&p), processor(p)
{
    ...
    addAndMakeVisible(exportKickButton);
    addAndMakeVisible(exportSnareButton);
    addAndMakeVisible(exportHatButton);
    exportKickButton.onClick  = [this] { exportSampleToFile(GrooveEngine::DrumClass::Kick); };
    exportSnareButton.onClick = [this] { exportSampleToFile(GrooveEngine::DrumClass::Snare); };
    exportHatButton.onClick   = [this] { exportSampleToFile(GrooveEngine::DrumClass::Hat); };
...
void PocketWorkAudioProcessorEditor::resized()
{
    ...
    exportKickButton.setBounds(216, 420, 80, 32);
    kickSampleLabel.setBounds(304, 425, 210, 22);

    loadSnareButton.setBounds(40, 460, 100, 32);
    clearSnareButton.setBounds(148, 460, 60, 32);
    exportSnareButton.setBounds(216, 460, 80, 32);
    snareSampleLabel.setBounds(304, 465, 210, 22);

    loadHatButton.setBounds(40, 500, 100, 32);
    clearHatButton.setBounds(148, 500, 60, 32);
    exportHatButton.setBounds(216, 500, 80, 32);
    hatSampleLabel.setBounds(304, 505, 210, 22);
...
void PocketWorkAudioProcessorEditor::loadBreakbeat()
{
    ...
    if (preselect == juce::File{})
        preselect = juce::File(processor.getMostRecentFile());
    ...
    GrooveEngine::DrumClass cls, juce::Label& targetLabel)
{
    ...
    browseForSampleWithPreview(
        ...
        *folderMember,
        [this, slot]() -> juce::File
        {
            juce::File f(processor.getFileForSlot(slot));
            return f != juce::File{} ? f : juce::File(processor.getMostRecentFile());
        }(),
...
void PocketWorkAudioProcessorEditor::exportSampleToFile(GrooveEngine::DrumClass cls)
{
    juce::String suggestedName = "sample.wav";
    switch (cls)
    {
        case GrooveEngine::DrumClass::Kick:  suggestedName = "kick_trimmed.wav";  break;
        case GrooveEngine::DrumClass::Snare: suggestedName = "snare_trimmed.wav"; break;
        case GrooveEngine::DrumClass::Hat:   suggestedName = "hat_trimmed.wav";   break;
    }

    auto startFolder = exportBrowseFolder.isDirectory()
        ? exportBrowseFolder
        : juce::File::getSpecialLocation(juce::File::userDocumentsDirectory);

    auto chooser = std::make_shared<juce::FileChooser>(
        "Export Sample As WAV",
        startFolder.getChildFile(suggestedName),
        "*.wav");

    chooser->launchAsync(
        juce::FileBrowserComponent::saveMode |
        juce::FileBrowserComponent::canSelectFiles,
        [this, chooser, cls](const juce::FileChooser& fc)
        {
            auto file = fc.getResult();
            if (file == juce::File{})
                return;

            const bool ok = processor.exportSample(cls, file);
            ...
            if (!ok)
            {
                juce::AlertWindow::showAsync(
                    juce::MessageBoxOptions()
                        ...
                        .withMessage("Nothing is loaded for this slot yet, or the "
