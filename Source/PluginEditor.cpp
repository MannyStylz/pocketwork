    ...
    : AudioProcessorEditor(&p), processor(p)
{
    ...
    // your own loaded kick/snare/hat samples. "Play MIDI" triggers ONLY
    // your samples, using the same extracted timing, without the
    // breakbeat audio underneath. Both loop until stopped, and starting
    // one automatically stops the other (they share the same playback
    // position, so running both at once wouldn't make sense).
    ...
    playBoth.onClick = [this]
    {
        ...
        processor.setPlaybackState(nowOn, true);
    };
    ...
    playMidi.onClick = [this]
    {
        ...
        processor.setPlaybackState(nowOn, false);
    };
    ...
    browseForSampleWithPreview(
        ...
        [this](const juce::File& file, double trimStart, double trimEnd)
        {
            ...
            processor.setFolderForSlot(
                PocketWorkAudioProcessor::BrowseSlot::Breakbeat,
                breakbeatBrowseFolder.getFullPathName());
