...
{
    lastBrowsedFolder = juce::File(processor.getLastBrowsedFolder());
    ...
    // "Play Both" plays the loaded breakbeat's real audio AND triggers
    // your own loaded kick/snare/hat samples in sync with each
    // detected hit — no MIDI-out/Patcher routing needed, everything
    // happens inside this one plugin.
...
void PocketWorkAudioProcessorEditor::resized()
{
    ...
    loadKickButton.setBounds(40, 420, 130, 32);
    kickSampleLabel.setBounds(180, 425, 340, 22);

    loadSnareButton.setBounds(40, 460, 130, 32);
    snareSampleLabel.setBounds(180, 465, 340, 22);

    loadHatButton.setBounds(40, 500, 130, 32);
    hatSampleLabel.setBounds(180, 505, 340, 22);
...
void PocketWorkAudioProcessorEditor::loadBreakbeat()
{
    browseForSampleWithPreview(
        lastBrowsedFolder,
        "Load Breakbeat Audio (click a file to preview it)",
        ...
        [this](const juce::File& file)
        {
            ...
            processor.setLastBrowsedFolder(lastBrowsedFolder.getFullPathName());
    ...
    GrooveEngine::DrumClass cls, juce::Label& targetLabel)
{
    browseForSampleWithPreview(
        lastBrowsedFolder,
        "Load Sample (click a file to preview it)",
        "*.wav;*.aif;*.aiff;*.mp3;*.flac",
        [this, cls, &targetLabel](const juce::File& file)
        {
            if (file == juce::File{})
                return;

            const bool ok = processor.loadSampleForClass(cls, file);
...
void PocketWorkAudioProcessorEditor::exportMidi()
{
    ...
    chooser->launchAsync(
        ...
        [this, chooser](const juce::FileChooser& fc)
        {
            ...
            if (ok)
            {
                lastBrowsedFolder = file.getParentDirectory();
                processor.setLastBrowsedFolder(lastBrowsedFolder.getFullPathName());
            }
