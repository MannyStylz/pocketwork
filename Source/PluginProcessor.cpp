...
void PocketWorkAudioProcessor::setPlaybackState(bool shouldPlay, bool includeBreakbeatAudio)
{
    if (shouldPlay)
        breakbeatPlayPosition.store(0);

    includeAudioInPlayback.store(includeBreakbeatAudio);
    ...
    juce::MidiBuffer& midi)
{
    ...
    //
    // Two modes, both looping continuously until stopped:
    // - "Play Both": copies the breakbeat's real audio into the output
    //   AND triggers your loaded samples at each detected hit.
    // - "Play MIDI": skips the breakbeat audio entirely, triggering
    //   ONLY your loaded samples — useful for judging how your own
    //   sounds carry the groove without the reference audio underneath.
    if (breakbeatPlaying.load() && breakbeatBuffer.getNumSamples() > 0)
    {
        ...
        const bool includeAudio = includeAudioInPlayback.load();
