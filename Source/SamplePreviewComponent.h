...
public:
    ...
    void playFile(const juce::File& file)
    {
        ...
        // CRITICAL ORDER: tell the transport to let go of the old source
        // FIRST (this safely synchronizes with the audio thread via
        // JUCE's internal locking), and only THEN destroy the object.
        // Destroying it first (the previous, buggy order) could let the
        // audio thread's callback still be touching the old object at
        // the exact moment it was being deleted — an intermittent
        // use-after-free crash, which is why this worked most of the
        // time but not always.
        transportSource.setSource(nullptr);
        readerSource.reset();
