...
//
// Takes a REFERENCE to a shared SamplePreviewPlayer rather than owning
// its own — repeatedly creating/destroying an audio device connection
// every time a Load dialog opens and closes was a real source of
// instability. One shared player lives for the plugin's whole session.
class SamplePreviewComponent : public juce::FilePreviewComponent,
                               private juce::Timer,
                               private juce::ChangeListener
{
public:
    explicit SamplePreviewComponent(SamplePreviewPlayer& sharedPlayer)
        : thumbnailCache(4),
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        {
            // Not near either handle — drag here to PAN the zoomed view
            // left/right, so you can reach both trim points precisely
            // without zooming back out.
            draggingHandle = 3;
            panDragStartX = e.x;
            panDragStartViewSeconds = viewStartSeconds;
        }
    }
    ...
    void mouseDrag(const juce::MouseEvent& e) override
    {
        ...
        {
            double total = thumbnail.getTotalLength();
            double secondsPerPixel = viewLengthSeconds / juce::jmax(1, waveformArea.getWidth());
            double deltaSeconds = (e.x - panDragStartX) * secondsPerPixel;

            viewStartSeconds = juce::jlimit(0.0, juce::jmax(0.0, total - viewLengthSeconds),
                                            panDragStartViewSeconds - deltaSeconds);
            repaint();
            return;
        }
...
private:
    ...
    int draggingHandle = 0; // 0 = none, 1 = start, 2 = end, 3 = pan
    int panDragStartX = 0;
    double panDragStartViewSeconds = 0.0;
...
public:
    ...
    SampleBrowserDialogContent(SamplePreviewPlayer& sharedPlayer,
        ...
        : onFileChosen(std::move(callback)),
          ...
          previewOwner(std::make_unique<SamplePreviewComponent>(sharedPlayer)),
...
inline void browseForSampleWithPreview(
    SamplePreviewPlayer& sharedPlayer,
                       ...
                       double trimEndSeconds)> onFileChosen)
{
    ...
    options.content.setOwned(
        new SampleBrowserDialogContent(sharedPlayer, startFolder, preselectFile,
