...
public:
    ...
    // Saves the currently loaded (and trimmed) sample out as its own WAV
    // file — lets you load it into a normal, separate FL Studio
    // instrument/channel per sound, sidestepping any plugin-routing
    // complexity entirely.
    bool exportSample(GrooveEngine::DrumClass drumClass, const juce::File& destFile);
    ...
    juce::String getMostRecentFile() const { return mostRecentLoadedFilePath; }
...
private:
    ...
    double kickSampleRate = 44100.0, snareSampleRate = 44100.0, hatSampleRate = 44100.0;

    // The single most recently loaded file across ANY slot — used as a
    // fallback default when opening a slot that has no history of its
    // own yet, since kick/snare/hat are often chopped from the same
    // source file/pack in one sitting.
    juce::String mostRecentLoadedFilePath;
