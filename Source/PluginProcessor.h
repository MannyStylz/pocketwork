...
public:
    ...
    // Remembers the last folder browsed for ANY sample/breakbeat/export,
    // saved with the plugin's state so it survives project reload too —
    // not just the current editing session.
    juce::String getLastBrowsedFolder() const { return lastBrowsedFolderPath; }
    void setLastBrowsedFolder(const juce::String& path) { lastBrowsedFolderPath = path; }
...
private:
    ...
    juce::String lastBrowsedFolderPath =
