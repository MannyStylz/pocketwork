...
public:
    ...
    // Playback modes: "Play Both" plays the breakbeat's real audio PLUS
    // triggers your loaded samples; "Play MIDI" triggers ONLY your
    // samples, using the exact same extracted timing, without the
    // original breakbeat audio underneath — useful for judging how your
    // own sounds carry the groove on their own. Both loop continuously
    // until stopped (click again to stop).
    void setPlaybackState(bool shouldPlay, bool includeBreakbeatAudio);

    // Each of Kick/Snare/Hat/Breakbeat/Export remembers ITS OWN last-used
    // folder independently — so re-opening "Load Snare" goes back to
    // wherever your snare came from, even if you loaded a kick from a
    // totally different folder in between. Saved with the plugin's state
    // so it survives project reload too.
...
private:
    ...
    std::atomic<bool> includeAudioInPlayback { true };
