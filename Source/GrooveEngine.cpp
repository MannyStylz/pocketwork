...
void GrooveEngine::getPlaybackHitsSnapshot(PlaybackHitsSnapshot& outSnapshot) const
{
    const juce::SpinLock::ScopedTryLockType lock(hitsLock);

    if (!lock.isLocked())
    {
        // Message thread is mid-update at this exact instant (rare,
        // brief) — skip using new data for just this one audio block
        // rather than ever blocking the audio thread to wait.
        outSnapshot.count = 0;
        return;
    }

    int n = juce::jmin(playbackHits.size(), kMaxPlaybackHitsSnapshot);
    for (int i = 0; i < n; ++i)
        outSnapshot.hits[static_cast<size_t>(i)] = playbackHits.getReference(i);
    outSnapshot.count = n;
}
...
{
    // Build results into LOCAL containers throughout this whole
    // (potentially slow) analysis, and only commit them to the real
    // member arrays at the very end under a brief lock. This is what
    // makes it safe: the audio thread can keep reading the OLD data
    // via getPlaybackHitsSnapshot() the entire time this function is
    // running, right up until the instant the new data is ready.
    juce::Array<Hit> newHits;
    juce::Array<PlaybackHit> newPlaybackHits;

    auto commitAndReturn = [&]
    {
        const juce::SpinLock::ScopedLockType lock(hitsLock);
        hits.swapWith(newHits);
        playbackHits.swapWith(newPlaybackHits);
    };

    const int numSamples = audio.getNumSamples();
    if (numSamples <= 0 || sourceSampleRate <= 0.0)
    {
        commitAndReturn(); // honestly commits "nothing detected" too
        return;
    }
    ...
    if (rawHits.empty())
    {
        commitAndReturn(); // Honest outcome: nothing detected, not a fake result.
        return;
    }
    ...
    auto classifyHit = [&](int windowIndex, float totalRms) -> DrumClass
    {
        ...
        // NOTE: the high-frequency measurement here uses a gentle
        // single-pole filter, which lets through much more energy than
        // a steep filter would — so realistic "highRatio" values run
        // much lower than intuition suggests. The original 0.45
        // threshold was calibrated wrong and made Hat essentially
        // unreachable (everything fell to Kick or Snare instead).
        if (lowRatio > 0.55)
