...
public:
    ...
    // Thread-safe snapshot for the audio thread — NEVER blocks. If the
    // message thread happens to be mid-update at the exact same moment
    // (rare, brief), this returns count=0 for just this one audio block
    // rather than risking a real-time-unsafe wait. This is the fix for
    // a genuine race condition: without this, the message thread
    // (Detect Groove, loading a new file) could rewrite this data at
    // the exact same instant the audio thread was reading it during
    // looping playback — a real, serious bug, not a rare edge case.
    static constexpr int kMaxPlaybackHitsSnapshot = 1024;
    struct PlaybackHitsSnapshot
    {
        std::array<PlaybackHit, kMaxPlaybackHitsSnapshot> hits;
        int count = 0;
    };
    void getPlaybackHitsSnapshot(PlaybackHitsSnapshot& outSnapshot) const;
    ...
    float getDynamics() const { return dynamicsAmount; }
...
private:
    ...
    mutable juce::SpinLock hitsLock;
