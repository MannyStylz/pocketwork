...
private:
    ...
    // Initialized from the processor's saved state (constructor body),
    // and written back to it after every successful browse — so it
    // survives project reload, not just the current editing session.
    juce::File lastBrowsedFolder;
