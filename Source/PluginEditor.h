...
private:
    ...
    // Remembers each slot's OWN last-used folder independently (Kick,
    // Snare, Hat, Breakbeat, Export all separate) — initialized from the
    // processor's saved state in the constructor, and written back to it
    // after every successful browse. Kept as members (not locals) since
    // the browse dialog is asynchronous and needs these to stay alive
