...
## v2.0 — the real fix for the save crash (confirmed cross-DAW)
- Found and fixed the actual cause of the save-crash-with-data-loaded
  bug: confirmed by reproducing in TWO completely different DAWs (FL
  Studio and Studio One), proving it was a genuine bug in our own code,
  not a host quirk. The audio-embedding code was handing off the address
  of a temporary (stack) variable to code that permanently takes
  ownership and later deletes it — undefined behavior that reliably
  crashes the moment there's real audio data to embed (which is exactly
  why an empty project always saved fine). Fixed by heap-allocating that
  object properly so ownership transfer is actually safe.
